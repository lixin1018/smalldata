function ExcelGridValidator(){
	...
	this.setCellRefEffects = function(destCellId, refCellId){
 		var cell = this.eg.getCell(refCellId); 
 		cell.addEffectCellId(destCellId);
	} 
	
	this.compileCellExp = function(eg, cellValueTypes, allSheetColRows, cellId, needRemoveEffectFromCell){
		this.eg = eg;
		...
		if(cell.isExp){
			...
			try{			
				...
				//List<ExpTreePart>
				var expParts = vd.getExpTreeParts(exp, cellValueTypes, allSheetColRows, sheetId); 
				var referNode = new CellReferanceNode(); 
				referNode.setCellId(cellId);
				this.checkReferance(referNode, expParts, allSheetColRows);	
				
				this.checkExpCycleRefer(cellId, referNode);	
	...
	//判断是否存在循环引用
	this.checkExpCycleRefer = function(checkCellId, referNode){ 
		var referCellIds = referNode.getReferCellIds();
		var referCellIdArray = new Array();
		var referCount = 0;
		for(var refCellId in referCellIds){
			if(refCellId == checkCellId){
				throw new Error("单元格公式不能引用当前单元格");
			}
			referCount++;
			referCellIdArray.push(refCellId); 
		}
		if(referCount > 0){
			this.checkHasReferCellInArray(referCellIdArray, checkCellId);
		}
	}
	
	//判断cellId对应的单元格是否引用了checkCellIdArray里的单元格
	this.checkHasReferCellInArray = function(checkCellIdArray, cellId){
		var cell = this.eg.getCell(cellId);
		var effectCellIds = cell.effectCellIds;		
		for(var effectCellId in effectCellIds){	  
			if(checkCellIdArray.contains(effectCellId)){
				throw new Error("单元格公式不能出现循环引用");
			}
			else{
				this.checkHasReferCellInArray(checkCellIdArray, effectCellId);
			}
		}
	}
	...
	this.checkReferance = function(referNode, expParts, allSheetColRows){
		for(var i = 0; i < expParts.length; i++){
			...
			if(expPart.getPartType() == partType.CellReferance){ 
				...
				var refCellId = expPart.getCRefSheetId() + "_" + cellRefInfo.getColId() + "_" + cellRefInfo.getRowId();
				referNode.getReferCellIds()[refCellId] = ""; 
			}
			else if(expPart.getPartType() == partType.RangeReferance){
				var cRefSheetId = expPart.getCRefSheetId();
				var sheetColRows = allSheetColRows[cRefSheetId]; 
				var columns = sheetColRows.columns;
				var rows = sheetColRows.rows;
				...
				var fromRowIndex = fromRowId == "" ? 0 : rows.getRowIndex(fromRowId);
				var toRowIndex = toRowId == "" ? (rows.size() - 1) : rows.getRowIndex(toRowId); 
				...
				for(var colIndex = fromColIndex; colIndex <= toColIndex; colIndex++){
					for(var rowIndex = fromRowIndex; rowIndex <= toRowIndex; rowIndex++){
						...
						var refCellId = cRefSheetId + "_" + colId + "_" + rowId;
						referNode.getReferCellIds()[refCellId] = "";
					}	
				}			 
			}
	...
	this.validateExp = function(vd, cellId, rootPart, cellValueTypes, columns, rows){
		try{
			var resultValueTypeStr = vd.getExpResultValueType(rootPart, null);
			var resultValueType = resultValueTypeStr.toLowerCase();
			this.eg.getCell(cellId).cellValueType = resultValueType;
...
function ExcelGridExpressionValidator(){ 
				 				...
				 				if(repeatParamCount % repeatParamCount  == 0){
				 					...
				 					for(var j = fromRepeatIndex; j < expFuncParamCount; j++){
				 						...
				 						if(paramType == "java.lang.Object"){
				 							var pIndex = (expFuncParamCount - fromRepeatIndex) % repeatParamCount;
				 							paramType = repeatableParamType[pIndex];
				 						}
				 						allRepeatParamValueTypeStr = allRepeatParamValueTypeStr + paramType + ";";
				 					}  
	...
	this.getExpTreeParts = function(exp, cellValueTypes, allSheetColRows, sheetId){
		...
		try{			
			...
			//识别单元格引用
			this.findCellReference(allParts, cellValueTypes, allSheetColRows, sheetId);
			...
			switch(expTreePart.getPartType()){ 
				case partType.CellReferance:
					if(expTreePart.getCellReferSourceType() == cellReferanceSourceType.CurrentSheet){
						...
						var cellId = expTreePart.getCRefSheetId() + "_" + cellReferInfo.getColId() + "_" + cellReferInfo.getRowId();
	...
	//从引用的串里，获取sheet的名称
	this.getSheetNameFromRef = function(refSheetName){
		if(refSheetName.startWith("'")){
			refSheetName = refSheetName.substr(1);			
		}
		if(refSheetName.endWith("'")){
			refSheetName = refSheetName.substr(0, refSheetName.length - 1);			
		}
		
		refSheetName = cmnPcr.replace(refSheetName, "''", "'");
		return refSheetName;		
	}
	this.getRefSheetNameFromName = function(sheetName){ 
		sheetName = cmnPcr.replace(sheetName, "'", "''");
		return sheetName;		
	}
	
	this.getSheetColRowsByName = function(name, allSheetColRows){
		for(var sheetId in allSheetColRows){
			var sheetColRows = allSheetColRows[sheetId];
			if(sheetColRows.name == name){
				return sheetColRows;
			}
		}
		return null;
	...
	this.findCellReference = function(allParts, cellValueTypes, allSheetColRows, sheetId){
		for(var i = 0; i < allParts.length; i++){
			var p = allParts[i];
			if(p.getPartType() == partType.Unknown){
				...
				var cSheetId = sheetId;		
				var sheetColRows = allSheetColRows[cSheetId];
				var cSheetName = sheetColRows.name;
				var columns = sheetColRows.columns;
				var rows = sheetColRows.rows;
				if(ps.length > 1){
					cSheetName = this.getSheetNameFromRef(ps[0]);
					sheetColRows = this.getSheetColRowsByName(cSheetName, allSheetColRows);
					cSheetId = sheetColRows == null ? "" : sheetColRows.id;
					columns = sheetColRows == null ? null :  sheetColRows.columns;
					rows =  sheetColRows == null ? null : sheetColRows.rows; 
				}
				
				p.setCRefSheetName(cSheetName);
				p.setCRefSheetId(cSheetId);
				
				var names = ps[ps.length - 1].split(":");
				if(names.length == 1){
					var cellName = names[0];
					var cellRefInfo = this.getCellReferenceInfo(cSheetId, cellName, columns, rows);
					if(cellRefInfo != null){
						if(cellRefInfo.getColName().length != 0 && cellRefInfo.getRowName().length != 0){
							...
							var cellId = cSheetId + "_" + colId + "_" + rowId;
							var cellValueType = cellValueTypes[cellId];
					...
					var fromCellRefInfo = this.getCellReferenceInfo(cSheetId, fromCellName, columns, rows);
					var toCellRefInfo = this.getCellReferenceInfo(cSheetId, toCellName, columns, rows);
	...
	this.getCellReferenceInfo = function(sheetId, cellName, columns, rows){
		var isCellRef = true; 
		var j = 0;
		...
		if(isCellRef){
			...
			if(isCellRef){
				...
				if(isCellRef){
					var cellRefInfo = new CellReferanceInfo(sheetId, colName, colId, colAbsolute, rowName, rowId, rowAbsolute);
					return cellRefInfo;
				}
				else{
					return null;
					...
					for(var i = 0;i < treeParts.length; i++){
						...
						if(i % 2 == 0){
							...
							if(vt != null){
								...
								parameterPartTypes.push(p.getPartType());
							}
							else{
						...
						if(opIndex < operatorIndexs.length){
							if(i == operatorIndexs[opIndex]){
								...
								if(i > 0){
									...
									for(var t = 0; t < tempTreeParts.length - 1; t++){
	...
	this.processBracket = function(allParts, errors){
			...
			if(p.getPartType() == partType.Bracket){
				if(p.getText() =="("){
					...
					if(stockParts.length == 0){
						...
						bracketPairs.push({beginPart:beginPart, endPart:p});
					}
				}
			}
		}
		if(stockParts.length != 0){
			...
			for(var i = 0; i < allParts.length; i++){
				...
				if(p.getPartType() == partType.Bracket){
					if(p.getText() == "("){					
						var endPart = bracketPairs[pairIndex].endPart;
						pairIndex++;
	...
	this.toExpPartJsCode = function(part){
		...
		switch(pPartType){
			...
			case partType.CellReferance:{
				var cRefSheetId = part.getCRefSheetId();
				var cellReferInfo = part.getCellRefInfo(); 
				return "runningEG.getCellValue(\"" + cRefSheetId + "\", \"" + cellReferInfo.getColId() + "\", \"" + cellReferInfo.getRowId() + "\")";
			}
			...
			case partType.RangeReferance:{
				...
				return "runningEG.getRangeValue(\"" + cRefSheetId + "\", \"" + fromCellReferInfo.getColId() + "\", \"" + fromCellReferInfo.getRowId() + "\", \"" + toCellReferInfo.getColId() + "\", \"" + toCellReferInfo.getRowId() + "\")";
			}
			default:{
...
function CellReferanceInfo(sheetId, colName, colId, colAbsolute, rowName, rowId, rowAbsolute){ 
	...
	this.clone = function(){
		var newCellRefInfo = new CellReferanceInfo(this.sheetId, this.colName, this.colId, this.colAbsolute, this.rowName, this.rowId, this.rowAbsolute);
	...
	this.setSheetId = function(sheetId) {
		this.sheetId = sheetId;
	}
...
function ExpTreePart(){
	...
	this.cRefSheetId = null;
	this.cRefSheetName = null;
	...
	this.clone = function(){
		...
		newPart.cRefSheetId = this.cRefSheetId;
		newPart.cRefSheetName = this.cRefSheetName;
	...
	this.getCRefSheetId = function() {
		return this.cRefSheetId;
	}
	this.setCRefSheetId = function(cRefSheetId) {
		this.cRefSheetId = cRefSheetId;
	}
	this.getCRefSheetName = function() {
		return this.cRefSheetName;
	}
	this.setCRefSheetName = function(cRefSheetName) {
		this.cRefSheetName = cRefSheetName;
	}
}
