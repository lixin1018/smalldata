...
<head> 
	...
	<script type="text/javascript" src="${dataModel}/dh_GobalVariableType.js"></script>
	<script type="text/javascript" src="${viewModel}/dh_GobalVariableType.js"></script>
	...
	<script> 
		$(document).ready(function(){ 
			var p = { 
						...
						multiselect:false,  
						dataModel:dataModels.dh_GobalVariableType,
						onePageRowCount:20,
						isRefreshAfterSave:false,
						viewModel:viewModels.dh_GobalVariableType 
			};
			var grid = new NcpGrid(p); 
			grid.show();

			$("#cleanVariableValueBtn").click(function(){ 
				var selRowId = $(grid.gridCtrl).jqGrid("getGridParam", "selrow");
				var currentRow = grid.datatable.rows(selRowId); 
			 	if(currentRow == null){
			 		msgBox.alert({info:"没有选定行."});
			 	}
			 	else{
			 		if(msgBox.confirm({info:"确定清除吗?"})){
				 		var variableName = currentRow.getValue("name");
				 		grid.doOtherAction({
							actionName:"cleanVariableValue",
							customParam:{variableName: variableName},
...
<body class="easyui-layout" style="width:100%;height:100%;" id = "testGridContainer">
	<div class="ncpGridToolbarContainer" data-options="region:'north',border:false">
		<span class="ncpGridToolbar">
			...
			<a id="cleanVariableValueBtn" href="#" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-tip'">清除变量值</a> 
		</span>
